...
<h2 id="warning-glossary" class="margin-top--md">Warning Glossary</h2>

The meaning of, and rationale behind the various checks are listed below. The 
plugin will link to these sections where appropriate in the Lume debug bar
report.
...
Images need alt-text for SEO and accessibility. 
...
Other structure warnings might be a missing `<h1>` tag.
...
document is missing something, like an important meta tag or title. 

The document doesn't need _fixing_, it's just _incomplete_. This is reported as an
error because it should not happen if using archetypes properly.