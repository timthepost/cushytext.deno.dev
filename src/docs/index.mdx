...
Once you have the repo, the following sets `LUME_DRAFTS` true and starts the Lume 
local server:
...
## Current Focus
 ...
 - Blog and Docs need a TOC plugin. Looking at `mdast-util-toc` which
   powers a remark plugin that I unfortunately can't use (I need one 
   that just sets the page `toc` object), which this one easily does.
   I should have a plugin soon.
