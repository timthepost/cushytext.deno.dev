...
<h2 id="current_focus">Current Focus (All must be done before `0.0.1` testing release)</h2>
 ...
 - Conductor plugin is no longer needed, just re-name it to TOC. We'll eventually have 
   some kind of generator plugin, but not yet.

 - Everything needed to be a remote theme (`mod.ts` for remote files, 
   `init.ts` to generate default content)

 - Interactive [content rating feature](/dev) is mostly done, but needs secured with 
   honeypot field and probably captcha. Should also use middleware-side validation.

 - Clean up & Proper-ize this docs installation to completely document the theme.
