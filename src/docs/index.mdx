...
## Current Focus
 ...
 - Tag Styles. I've been (ab)using button styles a little differently 
   in a few different places to see what feels best. None really work
   out responsively in mixed elements, so need to roll some.

 - Need to come up with a waypoint -> directory map that the `src/generator/*`
   scripts can use to look up paths. Those scripts need to be able to know 
   that `page.data.waypoint` correlates to `/path`, and some of the values
   set in `path/_data.yml`. So I'm thinking of using `generators/_data.ts` 
   that generates this, possibly from a manually-edited list for now. 
