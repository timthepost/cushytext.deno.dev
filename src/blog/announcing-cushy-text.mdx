...
navigation, feeds and more. I just called this one `%blog%` for simplicity;
it could have just as easily been `%main-blog%`, or `%opinions%`, or even 
`%shitposts%` if I'm really being honest with myself. All that matters is 
it becomes a friendly, unique way to scope content _in that directory_.

the `blog:` key is what actually turns on the archetype, with `blog.basedir`
being the most important setting.
