...
You can run multiple instances of whatever you have on the same site independently; this 
includes blogs, documentation, collections of pages, or whatever. They have separate 
folders, independent tags and independent navigation if you want.

This is accomplished through directory-level configuration and the use of special keys 
...
Right now, a single archive is generated for tags and authors. This will be configurable
per-instance by specifying the locations in `_data.yml`:

```yml
layout: layouts/post.vto
waypoint: "%gaming-blog%"

blog:
  basedir: /minecraft-blog/
  title: Cushy Text Blog
  recent: 5
  archive: /gaming-archive/
  authors: /author/
```

Hey, wait! _Speaking of documentation .._

## A Documentation / Wiki Engine Archetype Is Also Built-In!
...
Again, I picked `%docs%` for familiarity; it could have been `%theme-docs%`, or 
`%api-docs%` - however you want to 'grab' it when you build nav, feeds and content. What 
turns on the archetype is the presence of the `docs:` key in `_data.yml`. And, in this
instance, I'm using the blog's waypoint to show a few posts in the docs sidebar.
...
I mean really, [check out all these toppings](/docs/#plugin-status)!
